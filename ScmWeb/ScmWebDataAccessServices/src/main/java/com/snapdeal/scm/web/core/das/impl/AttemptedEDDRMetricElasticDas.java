...
/**
 * AttemptedEDDRMetricElasticDas : G4
 *
 * @author prateek
 */
@SuppressWarnings({"rawtypes", "unchecked"})
@Service("G4")
public class AttemptedEDDRMetricElasticDas extends AbstractElasticDasImpl implements ILastMileElasticDas {

    private static final Logger LOG = LoggerFactory.getLogger(AttemptedEDDRMetricElasticDas.class);

    public static final String ATTEMPTED_COUNT   = "attemptedcount";
    public static final String EDDR              = "eddr";
    public static final String BEFORE_EDDR       = "percentage_attempted_before_eddr";
    public static final String WITHIN_EDDR       = "percentage_attempted_within_eddr";
    public static final String AFTER_EDDR        = "percentage_attempted_after_eddr";
    ...
    public List<AggResult> findMetricData(ElasticFilter elasticFilter) {
        ...
        durationTypeDateRange.setColumnName(SubOrderDetailElasticColumn.ATTEMPTED_STATUS_DATE.getColumnName());
        ...
        switch (elasticFilter.getStage()) {
            case ONE:
                ...
                break;
            case TWO:
                aggResults = secondLevelAggregation(boolQuery, super.dateHistogramBuilder(ATTEMPTED_COUNT, durationTypeDateRange));
                break;
            case THREE:
                aggResults = thirdLevelAggregation(boolQuery, super.dateHistogramBuilder(ATTEMPTED_COUNT, durationTypeDateRange));
                break;
            case FOUR:
    ...
    private List<AggResult> secondLevelAggregation(BoolQueryBuilder boolQuery, DateHistogramBuilder dateHistogramBuilder) {
        ...
        for (InternalHistogram.Bucket deliveredCountBucket : super.dateHistogramBuckets(searchResponse, ATTEMPTED_COUNT)) {
            ...
            for (Terms.Bucket laneTypeBucket : laneTypeTerm.getBuckets()) {
                ...
                aggResult.addDataValue(ATTEMPTED_COUNT, totalLaneTypeDocCount);
    ...
    private List<AggResult> thirdLevelAggregation(BoolQueryBuilder boolQuery, DateHistogramBuilder dateHistogramBuilder) {
        ...
        for (InternalHistogram.Bucket deliveredCountBucket : super.dateHistogramBuckets(searchResponse, ATTEMPTED_COUNT)) {
            ...
            for (Terms.Bucket courierGroupBucket : courierGroupTerm.getBuckets()) {
                ...
                aggResult.addDataValue(ATTEMPTED_COUNT, totalCourierGroupDocCount);
    ...
    private Script getEDDRPercentageScript() {
        ...
        script.append("if(new java.util.Date(doc['").append(SubOrderDetailElasticColumn.EXPECTED_DELIVERY_DATE_RANGE_START.getColumnName())
                .append("'].value) > new java.util.Date(doc['").append(SubOrderDetailElasticColumn.ATTEMPTED_STATUS_DATE.getColumnName())
                .append("'].value)){ return \"").append(BEFORE_EDDR)
                .append("\";}; if(new java.util.Date(doc['").append(SubOrderDetailElasticColumn.ATTEMPTED_STATUS_DATE.getColumnName())
